...
#PI_TARGET  = 'udpout:10.113.51.25:14551' # 
PI_TARGET  = 'udpout:192.168.1.193:14551'# Pi’s IP
...
# Choose the servo channel that maps to your unused output (e.g., SERVO9):
SERVO_CH     = 9      # e.g., SERVO9 port
# Match Mission Planner intended levels (so Pi hysteresis works):
#   MP: SERVOx_MIN=1100, TRIM=1500, MAX=1900
#   Pi: PWM_RETRACT=1200, PWM_RELEASE=1800
PWM_ON       = 1900   # High band (>= 1800) triggers ON in Pi
PWM_OFF      = 1100   # Low band (<= 1200) triggers OFF in Pi
PWM_NEUTRAL  = 1500   # for other channels / placeholder
SERVO_HZ     = 10     # send SERVO_OUTPUT_RAW at 10 Hz

# Pi hysteresis reference (for comments / debug)
PWM_RELEASE  = 1800
PWM_RETRACT  = 1200
...
PHASES = [
    ...
    ("LANDED",         mavutil.mavlink.MAV_LANDED_STATE_ON_GROUND,  8, HOME_LAT+0.0004, HOME_LON+0.0002, 10),
    ("SECOND_TAKEOFF", mavutil.mavlink.MAV_LANDED_STATE_IN_AIR,     5, HOME_LAT+0.0003, HOME_LON+0.0002, 25),
    ("IN_AIR2",        mavutil.mavlink.MAV_LANDED_STATE_IN_AIR,     5, HOME_LAT+0.00045,HOME_LON+0.0002, 25),
    ("LANDING2",       mavutil.mavlink.MAV_LANDED_STATE_LANDING,    3, HOME_LAT+0.0005, HOME_LON+0.0002, 10),
    ("LANDED2",        mavutil.mavlink.MAV_LANDED_STATE_ON_GROUND,  8, HOME_LAT+0.0005, HOME_LON+0.0002, 10),
...
def send_heartbeat(endpt, armed, mode_name):
    mode = ID_TO_MODE.get(mode_name.upper(), 3)  # default AUTO
    base_mode = 0
...
# ---- SERVO_OUTPUT_RAW (match MP+Pi hysteresis) -------------------------------
def _send_servo_robust(c, t_us, port, values8, chan_pwm):
    """
    Prefer 16-field ArduPilot signature; fall back to 8-field per port.
    """
    try:
        ...
        # Fall back to classic 8-channel per port
        c.mav.servo_output_raw_send(
            t_us, port,
...
def send_servo_output(c, t_us, servo_on):
    """Emit SERVO_OUTPUT_RAW with our signal channel set to ON/OFF."""
...
def main():
    ...
    # Servo signal state (what FC outputs for Pi to read)
    servo_on = False
    ...
    for cycle in range(1, N_CYCLES + 1):
        ...
        for label, landed, dur, lat, lon, rel_alt in PHASES:
            # Event hooks to match real behavior:
            # - After landing (LANDED): set SERVO ON (>=1800)
            # - After takeoff (*TAKEOFF): start 2s timer; then set SERVO OFF (<=1200)
            if label == "LANDED" or label == "LANDED2":
                update_all_endpts(lambda e: send_statustext(e, "Landed — SERVO ON"))
                print(">>> SERVO ON (after landing)")
                servo_on = True
                takeoff_off_deadline = None
            elif "TAKEOFF" in label:
                takeoff_off_deadline = time.time() + 2.0
                update_all_endpts(lambda e: send_statustext(e, "Takeoff — will SERVO OFF in 2s"))
                print(">>> Takeoff detected — will SERVO OFF in 2s")

            if label == "FINAL_LANDING":
                update_all_endpts(lambda e: send_statustext(e, "Mission complete"))
            ...
            while (time.time() - start) < dur:
                ...
                # 2 s after takeoff -> SERVO OFF
                if takeoff_off_deadline is not None and now >= takeoff_off_deadline:
                    servo_on = False
                    takeoff_off_deadline = None
                    update_all_endpts(lambda e: send_statustext(e, "SERVO OFF (2s post-takeoff)"))
                    print(">>> SERVO OFF (2s post-takeoff)")

                # Periodic SERVO_OUTPUT_RAW
            ...
            if label == "FINAL_LANDING" and not did_disarm_announce:
                armed = False
                update_all_endpts(lambda e: send_statustext(e, "Auto disarm"))
                print(">>> Auto disarm")
